Shader "Shaders/RayMarching"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float dist1 = sphere(float4(_Object1, 3), p);
                float dist2 = cube(float4(_Object2, 2.25), p);
                
                float distPlane = plane(p);
                // return smin(smin(dist1, dist2, 0.5), dist3, 0.5);
                // return smin(smax(dist1, dist2, 0.5), dist3, 0.5);
                
                return min(min(dist1, dist2), distPlane);
            }
